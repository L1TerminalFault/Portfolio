...
export default function Projects() {
  return (
    <div className="flex w-full h-full slide-to-left items-center justify-center">
      <div className="flex w-full h-full max-w-380">
        <div className="relative flex flex-col gap-9 w-full h-full pt-30 pb-25 overflow-scroll scrollbar-hidden">
          {projectCategories.map((eachCat) => (
            <div
              key={eachCat.index}
              className={`relative flex flex-col w-full h-max gap-2 ${projects.find((project) => eachCat.index === project.category) ? "" : "hidden"}`}
            >
              <div className="text-gray-400 2xl:ml-36 ml-6 flex justify-center shadow-md shadow-black/50 w-max px-7 py-2 rounded-full backdrop-blur-xl bg-white/9 gap-2 z-40 items-center top-0 sticky">
                {eachCat.text}
                <GoDotFill className="text-[6px] text-center" />
                {
                  <i className="text-xs -translate-x-0.5">
                    {eachCat.index === 0
                ...
                {projects
                  ...
                  .map((project) => (
                    <TiltedCard
                      ...
                      scaleOnHover={1.02}
                    >
                      <div className="p-1.5 backdrop-blur-xl flex flex-col w-full group bg-white/5 rounded-3xl lg:rounded-4xl">
                        <div className="relative overflow-hidden w-full flex-1 h-full bg-[#ffffff09] items-center flex justify-center rounded-xl lg:rounded-3xl">
                          <Link
                            href={project.link ?? ""}
                            className={`${project.link ? "" : "hidden"} absolute top-3 right-3 backdrop-blur-xl bg-white/8 z-40 group-hover:px-4 shadow-lg shadow-black/40 p-2 flex items-center justify-center group-hover:gap-2 rounded-full group-hover:scale-110 hover:bg-white/9 transition-all`}
                          >
                            <LuArrowUpRight size={20} />
                            <div className="text-gray-200 text-xs max-w-0 group-hover:max-w-18 text-nowrap transition-all overflow-hidden">
                              Check it out
                            </div>
                          </Link>
                              ...
                              <div className="gap-2 flex-wrap flex">
                                {project.techStack
                                  ...
                                  .map((each) => (
                                    <div
                                      key={each.name}
                                      className="flex group-hover:gap-2 duration-500 transition-all gap-0 items-center group rounded-2xl overflow-hidden bg-white/0 backdrop-blur-md hover:bg-white/10 group-hover:pr-4 text-gray-400"
                                    >
                                      <Image
                                        alt=""
                                        src={each.icon}
